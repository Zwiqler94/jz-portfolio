...
const API_CASES = [
  ...
  },
  {
    ...
    base:
      'https://res.cloudinary.com/dhdioy0wn/search/7015ba7c91fa281e6c31282672d041c8558bd79dea54828efa5fd6d9fc230e51/300/eyJleHByZXNzaW9uIjoicmVzb3VyY2VfdHlwZTppbWFnZSBBTkQgYXNzZXRfZm9sZGVyOkpMWi1Qb3J0Zm9saW8vSmFwYW4iLCJtYXhfcmVzdWx0cyI6MjAwfQ==',
  },
] as const;
...
describe('ImageService', () => {
  ...
  it('formatForCloudinaryProvider handles relative paths', () => {
    const result = service.formatForCloudinaryProvider('images/gallery/photo.png');
    expect(result).toBe('/photo.png');
  });
  ...
  API_CASES.forEach(({ name, base }) => {
    it(`${name} uses the base Cloudinary query when no cursor is provided`, () => {
      invokeEndpoint(name).subscribe(() => { /* empty */ });
    ...
    it(`${name} appends the cursor when provided`, () => {
      invokeEndpoint(name, cursorKey).subscribe(() => { /* empty */ });
